...
class Kelas
{
    ...
    public function addKelas($nama, $user){
        ...
        if ($newID) {
            ...
            //------ Menulis LOG ---------
            $log    = $this->dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$user", "id_kelas"=>"$newID", "aksi"=>"5", "id_data"=>"$newID", "data"=>$insert, "date_created"=>date('Y-m-d H:i:s')));
        }else {
            $status     = "Failed";
        ...
        $update     = array("nama"=>$nama, "tentang"=>$tentang, "sekolah"=>$sekolah, "tkb"=>$tkb, "date_modified"=>date('Y-m-d H:i:s'));
          ...
          try {
              $this->table->update(array("_id" => new MongoId($kelas)), array('$set' => $update));
                ...
                //------ Menulis LOG ---------
                $log    = $this->dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$user", "id_kelas"=>"$kelas", "aksi"=>"10", "id_data"=>"$kelas", "data"=>$update, "date_created"=>date('Y-m-d H:i:s')));
    ...
    public function updateSekolahKelas($user, $sekolah, $kelas){
        $update     = array("sekolah"=>$sekolah, "update"=>1, "date_modified"=>date('Y-m-d H:i:s'));
        ...
        if (isset($query['_id'])) {
            if ($query['status'] == 'LOCKED') {
                ...
                if (!empty($query2["_id"])) {
                    ...
                    //------ Menulis LOG ---------
                    $log    = $this->dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$user", "id_kelas"=>$newID, "aksi"=>"7", "id_data"=>$newID, "date_created"=>date('Y-m-d H:i:s')));
                }
            }
        }else {
            $status     = "Failed";
    ...
    public function addPost($post, $kelas, $user){
        ...
        if ($newID) {
            ...
            //------ Menulis LOG ---------
            $log      = $this->dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$user", "id_kelas"=>"$kelas", "aksi"=>"14", "id_data"=>"$newID", "data"=>$insert, "date_created"=>date('Y-m-d H:i:s')));
        }else {
            $status     = "Failed";
        }
