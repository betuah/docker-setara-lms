...
if(!isset($_SESSION["start_time"])){
    ...
    echo "<script>window.location.href='create-ujian?id=".$_GET['id']."';</script>";
}

if(!isset($_COOKIE["total_nilai_quiz"])) {
    setcookie("total_nilai_quiz", 0, time() + (86400 * 30), "/ujian.php");
}
...
if(isset($_GET['submit'])){
    ...
    $quizClass->setInfoQuizPeserta($_GET['id'], $user, $sekolah, "1", "end");
...
?>
    <script type="text/javascript">document.title = "Ujian <?=$infoQuiz['nama']?> - seTARA daring";</script>
				...
				<div class="col-xl-3 col-lg-4">
					<aside id="menu-fixed2" class="profile-side">
						<section class="box-typical profile-side-user">
                            ...
                            <!-- <button id="submit-2" class="btn btn-rounded btn-primary" title="Ujian" data-toggle="popover" data-placement="right" data-trigger="hover" data-content="Tombol untuk mulai mengerjakan ujian." disabled><i class="font-icon font-icon-play"></i> Kumpulkan</button> -->
                            <button id="submit-2" class="btn btn-rounded btn-primary" title="Ujian" data-toggle="popover" data-placement="right" data-trigger="hover" data-content="Tombol untuk mulai mengerjakan ujian."><i class="font-icon font-icon-play"></i> Kumpulkan</button>
	...
	<script>
		...
		$(document).ready(function() {
			...
			function timeisUp() {
                ...
                swal({
                    ...
                    document.location.href="ujian.php?id=<?php echo $_GET['id']; ?>&paket=<?=$infoQuiz['id_paket']?>&submit="+total_nilai_quiz+"&jumlah_soal="+jumlah_soal+"&user=<?=$_SESSION['lms_id']?>";
                });
        ...
        $('#submit,#submit-2').on('click', function() {
            swal({
                ...
                window.onbeforeunload = null;
                document.location.href="ujian.php?id=<?php echo $_GET['id']; ?>&paket=<?php echo $_GET['paket']; ?>&submit="+total_nilai_quiz+"&jumlah_soal="+jumlah_soal+"&user=<?=$_SESSION['lms_id']?>";
            });
        });
