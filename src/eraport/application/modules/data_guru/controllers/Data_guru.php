...
class Data_guru extends CI_Controller {
	function __construct() {
		...
		$this->d['npsn'] = $this->session->userdata($this->sespre.'npsn');
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM m_guru WHERE npsn='$npsn'")->num_rows();
        ...
        $qdata = $this->db->query("SELECT
                                        ...
                                        WHERE a.npsn='".$npsn."' AND a.nama LIKE '%".$search['value']."%' ORDER BY a.nama DESC LIMIT ".$start.", ".$length."");
        ...
        foreach ($q_datanya as $d) {
            ...
            $data_ok[0] = $no++;
            $data_ok[1] = $d['nip'];
            $data_ok[2] = $d['jml_aktif'] > 0 ? $d['nama']." / <b><i>".$d['username']."</i></b> / <b><i>tutor123</i></b>" : $d['nama'];
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO m_guru (npsn,nama, nip, is_bk) VALUES ('".$npsn."','".$p['nama']."', '".$p['nip']."', '".$p['isbk']."')");
    ...
    public function aktifkan($id) {
        ...
        if (empty($detil_data)) {
            ...
            $this->db->query("INSERT INTO m_admin (npsn,username,password,level,konid,aktif) VALUES ('".$npsn."', '".$username_fix."', '".$password."', 'guru', '$id', 'Y')");

            $d['status'] = "ok";
            $d['data'] = "Username : ".$username_fix." berhasil diaktifkan..! Password default tutor123";
        }
