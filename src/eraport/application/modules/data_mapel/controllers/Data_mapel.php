...
class Data_mapel extends CI_Controller {
    ...
    public function datatable() {
        ...
        $q_datanya = $this->db->query("SELECT * FROM m_mapel WHERE nama LIKE '%".$search['value']."%' ORDER BY id ASC LIMIT ".$start.", ".$length."")->result_array();
        $data = array();
        $no = ($start+1);
        ...
        foreach ($q_datanya as $d) {
            $data_ok = array();
            $data_ok[] = $no++;
            $data_ok[] = $d['kelompok']=='A'?'Kelompok Umum':'Kelompok Peminatan';
    ...
    public function index() {
    	$this->d['p'] = "list";
        $this->d['p_kelompok'] = array("A"=>"Kelompok Umum","B"=>"Kelompok Khusus");
