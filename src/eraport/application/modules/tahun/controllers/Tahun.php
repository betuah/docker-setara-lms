...
class Tahun extends CI_Controller {
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM tahun WHERE npsn='$npsn'")->num_rows();

        $qdata = $this->db->query("SELECT * FROM tahun WHERE npsn='$npsn' AND tahun LIKE '%".$search['value']."%' ORDER BY tahun ASC LIMIT ".$start.", ".$length."");
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $cek_sudah_ada = $this->db->query("SELECT tahun FROM tahun WHERE npsn='$npsn' AND tahun = '".$p['tahun'].$p['semester']."'")->num_rows();

            if ($cek_sudah_ada > 0) {
                $d['status'] = "gagal";
                $d['data'] = "Data '".$p['tahun'].$p['semester']."' ini sudah ada";
            } else {
                $this->db->query("INSERT INTO tahun (npsn, tahun, aktif, nama_kepsek, nip_kepsek, tgl_raport, tgl_raport_kelas3) VALUES ('$npsn','".$p['tahun'].$p['semester']."','N','".$p['nama_kepsek']."', '".$p['nip_kepsek']."', '".$p['tgl_raport']."', '".$p['tgl_raport_kelas3']."')");
                $d['status'] = "ok";
