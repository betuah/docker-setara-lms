...
class Data_siswa extends CI_Controller {
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM m_siswa WHERE npsn='$npsn' AND nama LIKE '%".$search['value']."%' AND stat_data = 'A'")->num_rows();

        $q_datanya = $this->db->query("SELECT a.*,
                                        (SELECT COUNT(id) FROM m_admin WHERE level = 'siswa' AND konid = a.id) AS jml_aktif
                                        FROM m_siswa a
                                        WHERE a.npsn='$npsn' AND a.nama LIKE '%".$search['value']."%' AND stat_data = 'A'
                                        ORDER BY a.nis ASC
    ...
    public function aktifkan($id) {
        ...
        if (empty($detil_data)) {
            ...
            $this->db->query("INSERT INTO m_admin (npsn,username,password,level,konid,aktif) VALUES ('".$npsn."', '".$username."', '".$password."', 'siswa', '$id', 'Y')");
    ...
    public function import_siswa() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            $strq = "INSERT INTO m_siswa (npsn, nis, nisn, nama, jk, tmp_lahir, tgl_lahir, agama, status, anakke, alamat, notelp, sek_asal, sek_asal_alamat, diterima_kelas, diterima_tgl, diterima_smt, ijazah_no, ijazah_thn, skhun_no, skhun_thn, ortu_ayah, ortu_ibu, ortu_alamat, ortu_notelp, ortu_ayah_pkj, ortu_ibu_pkj, wali, wali_alamat, notelp_rumah, wali_pkj) VALUES '$npsn',";
