	...
	<script>
		...
		if(jumlah_siswa == 0){
			...
			table = $('#perkembangan').DataTable( {
			    "processing"    : true,
			    "bServerSide"   : true,
			    "sAjaxSource"   : "url-API/Kelas/Mapel?action=perkembangan&status=<?=$_SESSION['lms_status']?>&akun=<?=$_SESSION['lms_id']?>&kelas=<?=$infoMapel['id_kelas']?>&tkb=<?=$idtkb?>&modul=<?=$idmodul?>&tugas=<?=$list_id_tugas?>&ujian=<?=$list_id_ujian?>&bobot_materi=<?=$infoModul['nilai']['materi']?>&bobot_tugas=<?=$infoModul['nilai']['tugas']?>&bobot_ujian=<?=$infoModul['nilai']['ujian']?>",
			    "deferRender"   : true,
			    "aoColumns"     : [
			                        { "mDataProp": "nama" },
			                        { "mDataProp": "tkb"},
