...
if(isset($method['action'])){
	...
	if($method['action'] == 'lockKelas'){
        ...
        if (isset($catch['_id'])) {
            ...
            try {
                ...
                //------ Menulis LOG ---------
                $log    = $dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$userID", "id_sekolah"=>$_SESSION['lms_sekolah'], "id_kelas"=>"$ID", "aksi"=>"9", "id_data"=>"$ID", "date_created"=>date('Y-m-d H:i:s')));
    ...
    if($method['action'] == 'rmv'){
        ...
        if ($query['creator'] == $method['u']) {
            ...
            //------ Menulis LOG ---------
            $log    = $dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$method[u]", "id_sekolah"=>$_SESSION['lms_sekolah'], "id_kelas"=>"$method[ID]", "aksi"=>"6", "id_data"=>"$method[ID]", "date_created"=>date('Y-m-d H:i:s')));
    ...
    if($method['action'] == 'removeAnggota'){
        ...
        //------ Menulis LOG ---------
        $log    = $dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$method[u]", "id_sekolah"=>$_SESSION['lms_sekolah'], "id_kelas"=>"$method[kelas]", "aksi"=>$aksi, "id_data"=>"$method[ID]", "date_created"=>date('Y-m-d H:i:s')));
    ...
    if($method['action'] == 'cPriv'){
        ...
        try {
            ...
            //------ Menulis LOG ---------
            $log    = $dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$method[u]", "id_sekolah"=>$_SESSION['lms_sekolah'], "id_kelas"=>"$method[kelas]", "aksi"=>"46", "id_data"=>"$method[ID]", "date_created"=>date('Y-m-d H:i:s')));
        } catch(MongoCursorException $e) {
            $status     = "Failed 1.";
    ...
    if($method['action'] == 'cTKB'){
        ...
        try {
            ...
            //------ Menulis LOG ---------
            $log    = $dbLog->log_action->insert(array("IP_address"=>$_SERVER['REMOTE_ADDR'], "id_user"=>"$method[u]", "id_sekolah"=>$_SESSION['lms_sekolah'], "id_kelas"=>"$method[kelas]", "aksi"=>"13", "id_data"=>"$method[ID]", "date_created"=>date('Y-m-d H:i:s')));
        } catch(MongoCursorException $e) {
            $status     = "Maaf";
    ...
    if($method['action'] == 'anggotaKelas'){
        ...
        foreach ( $cursor[0]['paginatedResults'] as $doc ) {
            ...
            $infoSekolah = $sekolahClass->getInfoSekolah($doc['sekolah']);

            if(isset($infoSekolah) && !empty($infoSekolah)){
                $doc['sekolah'] = $infoSekolah['nama_sekolah_induk'];
            }
