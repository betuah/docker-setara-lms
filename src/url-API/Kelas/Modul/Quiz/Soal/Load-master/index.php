<?php
    ...
    foreach ( $cursor as $doc ) {
        ...
        $jawaban_user       = $soalClass->getOpsiJawabanUser($_SESSION['lms_id'], $id_quiz, $doc['_id']);
        $list_opsi_soal     = $soalClass->getListOpsiSoal($doc['_id'], $random_opsi);

        foreach ($list_opsi_soal as $opsi_soal) {
            if($opsi_soal['_id'] == $jawaban_user['id_opsi_soal']){
                $checked    = "checked";
            }else{
                $checked    = "";
            }
            $opsi           .= "<div class='radio'>
                                    <input type='radio' name='".$doc['_id']."' id='".$opsi_soal['_id']."' value='".$opsi_soal['_id']."' onclick=save_answer('".$id_quiz."','".$doc['_id']."','".$opsi_soal['_id']."') ".$checked.">
