...
class Sekolah
{
    ...
    public function GetAll($skip, $limit, $criteria, $order)
    {
        ...
        $getSekolah = empty($criteria) ? 
                                // $this->table->find(array('program'=>'SMA Rujukan'))->sort(array('npsn'=>1)) :
                                $this->table->find(array())->skip($skip)->limit($limit)->sort(array('npsn'=>1)) :
                                // $this->table->find(array())->skip($skip)->limit($limit)->sort($sorting) :
        ...
        if (empty($filter)) {
			...
			usort($seluruh, function($a, $b) {
				return $a[4] - $b[4];
			});
        }

		$feedback   = ['filter'=>$filter, 'total'=>$total, 'data'=>$seluruh];
        // $feedback   = ['filter'=>$jumlah_sekolah, 'total'=>$jumlah_sekolah, 'data'=>$seluruh];
	...
	// public function GetAllUser($user)
    public function GethvClass($skip, $limit, $criteria, $order)
    {
        $columns = array(
            0   => 'npsn' ,
            1   => 'nama_sekolah_induk',
        ...
        $getSekolah = empty($criteria) ? 
                                $this->table->find()->skip($skip)->sort($sorting) :
                                $this->table->find($criteria)->skip(intval($skip))->sort($sorting);
        // $getSekolah = $this->table->find()->sort($sorting);
        $total      = $this->table->find()->count();
        $filter     = $getSekolah->count();
