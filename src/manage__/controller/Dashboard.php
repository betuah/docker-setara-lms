...
class Dashboard
{
    ...
    {
        $query = $this->db->user->find(array('status'=>'guru'));
        $guru  = $query->count();

        $query = $this->db->user->find(array('status'=>'siswa'));
        $siswa = $query->count();

        $query = $this->db->kelas->find();
        $kelas = $query->count();

        $query = $this->db->modul->find();
        $modul = $query->count();
        
        $data  = ['jmlGuru'=>$guru,'jmlSiswa'=>$siswa,'jmlKelas'=>$kelas,'jmlModul'=>$modul];
