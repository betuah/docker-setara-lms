...
class Kelas
{
	...
	public function getClasslistBySchool($id)
    {
		...
		if($jmlKelas > 0)
		{
			$uwala['jumlah'] = $jmlKelas;
            $tampung = array();
			$a = 0;
            foreach ($query as $value)
			{
				$tampung[$a]['id']		= "$value[_id]";
				$tampung[$a]['kode']	= $value['kode'];
				$tampung[$a]['nama']	= $value['nama'];
				$a++;
			}
	...
	public function getMemberlistByClass($kelas,$peran)
    {
		
		$filter		= ($peran != 0) ? array("id_user"=>array('$ne' => ""), "id_kelas" => "$kelas", "status"=>"$peran") : array( "id_user"=>array('$ne' => ""), "id_kelas" => "$kelas" );
        $query		= $this->db->anggota_kelas->find($filter);
        $jmlMember	= $query->count();
		$uwala = array();
		
		if($jmlMember > 0)
		{
			$uwala['jumlah'] = $jmlMember;
            $tampung = array();
			// $a = 0;
            ...
            foreach ($query as $value)
			{
				$member  = $this->db->user->findOne(array("_id" => new MongoId($value['id_user'])));
				...
				// $tampung[$a]['id']      = "$member[_id]";
                // // $tampung[$a]['name']    = "<b>".$member['nama']."</b> (".$member['username'].")";
                // $tampung[$a]['nama']    = $member['nama'];
                // $tampung[$a]['user']    = $member['username'];
				// $a++;
			}
			
			// array_multisort($tampung, SORT_ASC);
			
			$this->array_sort_by_column($tampung, 'nama');
			
			// $uwala['list'] = $tampung;
			
			// array_multisort(array_map(function($tampung) {
    ...
    public function getlistBySchool($id, $peran)
    {
        ...
        if($jmlKelas > 0){
            ...
            foreach ($query as $index => $value) {
                $data   = $value;
                if ($peran != 0) {
                    $filter = array( "id_user"=>array('$ne' => ""), "id_kelas" => "$value[_id]", "status"=>"$peran");
                }else{
                    $filter = array( "id_user"=>array('$ne' => ""), "id_kelas" => "$value[_id]" );
    ...
    public function GetLog($iduser,$idkelas,$waktu)
    {
        ...
        $getdataLog = $collection->find(array(
										'id_user'=>$iduser,
										'id_kelas'=>$idkelas,
										'date_created'=>array(
											'$regex' => new MongoRegex("/^$waktu/i")
										)))->sort(array(
													'date_created'=> -1
										));
        ...
        foreach ($getdataLog as $value) {
            if (($value['id_kelas'] != '0') AND ($value['id_kelas'] != '')) {
				...
				$var = array(
						"name"	=> '<b>'.$duser.'</b>',
						"kelas"	=> $dkelas.' ('.$hak['posisi'].') ',
						"link"	=> "<a href='".$value['link']."' style='text-decoration: none'>Membuka halaman ".$value['halaman']."</a>",
						"selisih" => $selisih,
						"tanggal" => date_format(date_create($value['date_created']), "d-M-Y H:i:s")
				);
		...
		$this->array_sort_by_column($tampung, 'tanggal');

        $feedback   = ['filter'=>$filter, 'total'=>$total, 'data'=>$tampung];
	...
	public function GetLogKelas($kelas, $peran, $waktu)
    {
        // $tgl	= explode("-", $waktu);
        // $awal   = date('Y-m-d 00:00:00', strtotime($tgl[0]));
        // $akhir  = date('Y-m-d 23:59:59', strtotime($tgl[1]));
        // Informasi tentang User
        // $dataUser   = $this->db->user->findOne(array('_id' => new MongoId($iduser)));
        // $listKelas  = $this->getAnggotadikelas($iduser);
		$dataKelas  = $this->db->kelas->findOne(array('_id'=>new MongoId($kelas)));
		...
		if($peran == "0"){
			$anggota	= $this->db->anggota_kelas->find(array('id_kelas'=>"$kelas"));
		}else{
			$anggota	= $this->db->anggota_kelas->find(array('id_kelas'=>"$kelas", 'status'=>"$peran"));
		}
		
		foreach($anggota as $pengguna){
			$dataUser   = $this->db->user->findOne(array('_id' => new MongoId($pengguna['id_user'])));
			if ($pengguna['status'] == 1) {
                $role = 'Administrator Kelas';
            }elseif ($pengguna['status'] == 2) {
                $role = 'Guru Mata Pelajaran';
            }elseif ($pengguna['status'] == 3) {
                $role = 'Tutor';
            }else{
                $role = 'Siswa';
            }
            ...
            if ($getdataLog->count() > 0) {
                ...
                foreach ($getdataLog as $value) {
                    ...
                    $data['kelas']  = $dataKelas['nama']." <b>($role)</b>";
                    $data['durasi'] = sprintf('%02d Jam %02d menit %02d detik', ($selisih/3600),($selisih/60%60), $selisih%60);
                    $data['link'] =  "<a href='new_.php?id=$pengguna[id_user]&kelas=$kelas&waktu=$waktu' class='btn btn-sm'><i class='fa fa-search'></i></a>";
                }
