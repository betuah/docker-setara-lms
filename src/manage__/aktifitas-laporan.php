		...
		<div class="container-fluid">
			...
			<section class="card">
				<div class="card-block">
					<h5 class="with-border">Unduh Laporan berdasarkan</h5>
					...
					<div class="row">
							...
							<fieldset class="form-group">
								...
								<?php
									if ($_SESSION['admin_status'] == 'superadmin') {
										...
										echo '<select name="ID" id="sekolah1" class="form-control" data-live-search="true" required>';
						...
						<div class="col-md-5">
							<fieldset class="form-group">
								<label for="ajax-select semibold form-label" class="form-control-label">Pilih Kelas</label>
					...
					<div class="row">
						<!--<div class="col-md-6">
							<fieldset class="form-group">
								<label for="ajax-select semibold form-label" class="form-control-label">Nama Pengguna</label>
	...
	$( "#sekolah1" ).change(function() {
		if ($(this).val()) {
			...
			$.ajax({
				type: 'POST',
				url: 'API/Kelas/',
				data: {"action": "getClassbySchool", "ID": $(this).val(), "peran":$('#listperan').val()},
				success: function(res) {
					...
					if (res.data.jumlah) {
						$.each(res.data.list,function(index, value){
							isi += '<option value="'+value.id+'">'+value.nama+' (<b>'+value.kode+'</b>) </option>';
						});
					}
	...
	// $( "#kelas" ).change(function() {
		// if ($(this).val()) {
			...
			// $.ajax({
				// type: 'POST',
				// url: 'API/Kelas/',
				// data: {"action": "getMemberbyClass", "kelas": $('#kelas').val(), "peran":$('#listperan').val()},
				// success: function(res) {
					// // console.log(res.data.listSeluruh.length);
					// var isi = '<option value="">-- Pilih Salah Satu --</option>';
					// // for (var i = 0; i < res.data.jumlah; i++) {
					...
					// // }
					// if (res.data.jumlah) {
						// // res.data.list.forEach(function(entry) {
							// // console.log(entry);
						// // });
						// $.each(res.data.list,function(index, value){
							// isi += '<option value="'+value.id+'">'+value.nama+'</option>';
						// });
					// }
