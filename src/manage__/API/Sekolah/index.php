...
if(isset($method['action'])){
    ...
    if($method['action'] == 'getAll'){
        ...
        if (!empty($search)) {
            $criteria   = array(
                                '$or' => array(
                                            array('nama_sekolah_induk'=>new MongoRegex( '/'.$search.'/i')),
                                            array('npsn'=>new MongoRegex( '/'.$search.'/i')),
                                            array('program'=>new MongoRegex( '/'.$search.'/i'))
                                )
                            );
        }else{
            $criteria   = '';
        ...
        $data           = $userClass->GetAll($skip, $limit, $criteria, $order);
    ...
    if($method['action'] == 'getSome'){
        $sekolahClass      = new Sekolah();

        $list           = $method['list'];
        $draw           = $method['draw'];

        $data           = $sekolahClass->GetSome($list);
	...
	if($method['action'] == 'gethvClass'){
        ...
        if (!empty($search)) {
            $criteria   = array(
                                '$or' => array(
                                            array('nama_sekolah_induk'=>new MongoRegex( '/'.$search.'/i')),
                                            array('npsn'=>new MongoRegex( '/'.$search.'/i')),
                                            array('kab_kot'=>new MongoRegex( '/'.$search.'/i'))
                                )
                            );
        }else{
            $criteria   = '';
        ...
        $data           = $userClass->GethvClass($skip, $limit, $criteria, $order);
